...
Manager (`RSPM`) or the Microsoft R Archive Network (`MRAN`) or the CRAN
...
(`RSPM`) and the [Microsoft R Archive Network](https://mran.microsoft.com/)
(MRAN) allows installations of packages from their respective snapshot
...
to either `RSPM` or `MRAN`. By default, the package uses `RSPM` snapshots tied
...
with those in either the `RSPM` or `MRAN` repository.
